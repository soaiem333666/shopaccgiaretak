		...
		public function wp_nav_menu_objects( $items ) {
			foreach ( $items as &$item ) {
				if ( 'my-wallet' === $item->post_name && get_post_meta( $item->ID, '_show_wallet_icon_amount', true ) ) {
					$item->title = apply_filters( 'wp_wallet_nav_menu_title', '<img src="http://localhost:8888/game/wp-content/uploads/2023/03/wallet-filled-money-tool-1.png" style="width: 20px" alt="wallet icon"> ' . woo_wallet()->wallet->get_wallet_balance( get_current_user_id() ), $item );
				}
			}
			return $items;
		}
